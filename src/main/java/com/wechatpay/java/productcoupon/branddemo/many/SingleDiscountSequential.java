 ...
 * type: DISCOUNT（折扣券）
 */
public class SingleDiscountSequential {
  ...
  public static CreateProductCouponRequest buildRequest() {
    ...
    request.setOutRequestNo("MANY_SINGLE_DISCOUNT_20250101_001");
    // 必填，品牌ID，由微信支付分配
    request.setBrandId("120344");
    // 选填，商户侧商品券唯一标识
    request.setOutProductNo("Product_1234567892");
    ...
    // 必填，商品券名称，最多12个字符
    displayInfo.setName("指定商品8折券(可用3次)");
    // 必填，商品券图片URL
    ...
    request.setStockBundle(createStock("单品8折批次", 0, 20));
  ...
  private static StockForProgressiveBundle createStock(String remark, long threshold, long percentOff) {
    ...
    progressiveBundleUsageRule.setDiscountCouponList(new ArrayList<>(Arrays.asList(
        discountCoupon1, discountCoupon2, discountCoupon3)));
